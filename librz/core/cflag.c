...
static bool __tableItemCallback(RzFlagItem *flag, void *user) {
	...
	if (!RZ_STR_ISEMPTY(flag->name)) {
		...
		const char *addr = rz_str_newf("0x%08" PFMT64x, flag->offset);
		rz_table_add_row(t, addr, rz_str_newf("%" PFMT64d, flag->size), spaceName, flag->name, NULL);
	}
	return true;
}
...
static void flag_print(RzFlag *f, RzCmdStateOutput *state, ut64 range_from, ut64 range_to, bool in_range) {
	...
	case RZ_OUTPUT_MODE_TABLE: {
		...
		rz_table_add_column(state->d.t, typeNumber, "addr", 0);
		rz_table_add_column(state->d.t, typeNumber, "size", 0);
		rz_table_add_column(state->d.t, typeString, "space", 0);
		rz_table_add_column(state->d.t, typeString, "name", 0);
